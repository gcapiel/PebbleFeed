...
/* need to add logic to make sure we are on the same day */

  diffMinFirst = (9 * 60 + 11) - (tick_time->tm_hour * 60 + tick_time->tm_min);
  diffMinSecond = (9 * 60 + 32) - (tick_time->tm_hour * 60 + tick_time->tm_min);

  if (diffMinFirst == 2 && buzzOne == 0) {
  	vibes_short_pulse();
  	buzzOne = 1;
  }

  if (diffMinSecond == 2 && buzzTwo == 0) {
  	vibes_short_pulse();
  	buzzTwo = 1;
  }
