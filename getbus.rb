...
def replaceTimes(sourceCode, index)
	...
	if muniLine != "0" then
		...
		nextTime = adjust_time(Time.at(r.stop_at(ARGV[0].scan(/[0-9]+/)[1]).predictions.map(&:epochTime)[index].to_i/1000).utc)
...
def insertSchedule(sourceCode)
	...
	currentYear = adjust_time(Time.now).to_datetime.year()
	currentMon = adjust_time(Time.now).to_datetime.mon()
	currentDay = adjust_time(Time.now).to_datetime.day()
	...
	todaysItems.each_with_index do |calEvent, index|
		...
		eventStart = adjust_time(calEvent.start.to_time.utc)
...
sourceCode = File.read("/root/BusTimer/src/example_template.txt", :encoding => "UTF-8")
